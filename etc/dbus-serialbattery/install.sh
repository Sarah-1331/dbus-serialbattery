...
if [ "$version" = "latest release (recommended)" ]; then
    ...
    curl -s https://api.github.com/repos/sarah-1331/dbus-serialbattery/releases/latest | grep "browser_download_url.*gz" | cut -d : -f 2,3 | tr -d \" | wget -O /tmp/venus-data.tar.gz -qi -
fi
...
if [ "$version" = "nightly build" ]; then
    ...
    # download driver
    wget -O $branch.zip https://github.com/sarah-1331/dbus-serialbattery/archive/refs/heads/$branch.zip
    if [ $? -ne 0 ]; then
